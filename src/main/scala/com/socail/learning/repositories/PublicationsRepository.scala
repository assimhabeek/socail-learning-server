...
class PublicationsRepository(override val config: DatabaseConfig[JdbcProfile])
  ...
  def findByPageAndFriend(
                           ...
                           year: Option[String],
                         ): Future[(Int, Seq[(Publication, User, Int, Int)])] = {
    val query = publications
      .join(users).on((x, y) => x.userId === y.id && y.id =!= id)
      .join(friends).on((x, y) => (x._2.id === y.senderId || x._2.id === y.receiverId) && (y.receiverId === id || y.senderId === id) && y.state === 3)
      .joinLeft(specialties).on(_._1._1.spcialtyId === _.id)
      .filter(qry => List(
        textFilter.map(s => qry._1._1._1.title.indexOf(s) >= 0 ||
          qry._1._1._1.description.getOrElse("(^_^)").indexOf(s) >= 0),
        date.map(s => qry._1._1._1.date <= new Timestamp(System.currentTimeMillis()) && qry._1._1._1.date >=
          Try(Timestamp.valueOf(s))
            .getOrElse(Timestamp.valueOf("1900-01-01 1:00:00"))),
        categoryId.map(i => qry._1._1._1.categorieId.getOrElse(0) === Try(i.toInt).getOrElse(0)),
        specialtyId.map(i => qry._1._1._1.spcialtyId.getOrElse(0) === Try(i.toInt).getOrElse(0)),
        moduleId.map(i => qry._1._1._1.moduleId.getOrElse(0) === Try(i.toInt).getOrElse(0)),
      ...
      ).sortBy(x => x._1._1._1.date.desc)
    db.run(
      query.drop(page * 10).take(10).map(x => {
        (x._1._1._1, x._1._1._2,
          opinions.filter(y => y.publicationId === x._1._1._1.id && y.opinion === OpinionOptions.OPTION_LIKED).length,
          opinions.filter(y => y.publicationId === x._1._1._1.id && y.opinion === OpinionOptions.OPTION_DISLIKED).length)
      }).result.map(y => query.length.result.map(i => (i, y))).flatten
    ).recover {
      case e: Exception =>
  ...
  def findByPage(page: Int,
    ...
    val query = (publications join users on (_.userId === _.id) joinLeft specialties on (_._1.spcialtyId === _.id))
      .filter(qry => List(
    ...
    db.run(query.drop(page * 10).take(10).map(x => {
      (x._1._1,
        x._1._2,
